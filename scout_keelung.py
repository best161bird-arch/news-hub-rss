...
def write_to_hub(title, content, category, images, source, original_link):
    ...
    row_id = f"GOV-KEELUNG-{timestamp}"
...
def scout_keelung():
    """偵察基隆市政府 Web"""
    print("[3] 偵察基隆市政府 Web (標準模式)...")
    
    base_url = "https://www.klc.gov.tw"
    news_url = "https://www.klc.gov.tw/Resource/Api/News?$top=20&$orderby=PublishDate%20desc"
    ...
    for item in news_list[:5]:
        ...
        if write_to_hub(title, content, "地方", "", "基隆市政府", full_link):
            print(f"    ✅ 已寫入: {title[:25]}...")
            count += 1
    
    print(f"    基隆新增: {count} 篇")
    return count
